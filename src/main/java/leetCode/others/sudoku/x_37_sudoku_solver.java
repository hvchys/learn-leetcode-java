...
class Rule1 {
    ...
    static int[] idx_0_left = getPossibleIdxs(0);
    static int[] idx_1_left = getPossibleIdxs(1);
    static int[] idx_2_left = getPossibleIdxs(2);
    ...
    public static boolean applyOnRow(Integer[][] boardInt, int i){
        ...
        for(int x = 1; x < 10; x++){
            ...
            if(occurTime == 2){
                ...
                // 它们属于 block(i, j_temp)
                if(checkUpdateColX(x, i_real, i, j_temp, boardInt)){
                    return true; // 成功添加了一个
                }
            }
        }
    ...
    public static boolean applyOnCol(Integer[][] boardInt, int j){
        ...
        for(int x = 1; x < 10; x++){
            ...
            if(occurTime == 2){
                ...
                // 它们属于 block(i_temp, j)
                if(checkUpdateX(x, i_real_start, j_real, i_temp, j, boardInt)){
                    return true; // 成功添加了一个
                }
            }
        }
    ...
    public static boolean checkUpdateColX(int x, int cur_i, int block_i, int block_j, Integer[][] boardInt){
        /*
    ...
    public static int[] getPossibleIdxs(int cur_i){
        // 返回 block 是 cur_i行 之外的所有可能行
        int[] ans = new int[6];
        // cur_i*3: 0,1,2
        int lower = cur_i*3;
        int upper = cur_i*3 + 2;
